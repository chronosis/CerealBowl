...
class JObjStream {
  constructor(buff) {
    ...
    this.handles = {};
    this.classes = {}
    this.objectClasses = require('./objects')(this);
  }
  ...
  _readClassField() {
    ...
    switch (type) {
      ...
      case constants.DATATYPE_ARRAY:
        ...
        break;
      default:
        this._reportError(errors.STREAM_BAD_TYPE_DATA, '0x' + Buffer.from([type]).toString('hex'));
        break;
    }
  }
  ...
  _readClassData(classDef) {
    let object = _.clone(classDef);
    if (classDef !== null) {
      // Read in the actual data for the fields
      for (let name of classDef.fields) {
        console.log(name);
        let field = classDef.data[name];
        object.data[name].value = this._readData(field);
      }
  ...
  _readObject() {
    ...
    this._updateHandle(object.handle, object);
    return object;
  }
  ...
  _readBlock(typeFilter) {
    ...
    this._reportPosition();
    if (typeof typeFilter !== 'object') { throw (errors.FILTER_NOT_ARRAY); }
    tag = this._readTag();
    if (typeFilter.lastIndexOf(tag) === -1) {
      this._reportError(errors.STREAM_BAD_BLOCK_DATA, '0x' + Buffer.from([tag]).toString('hex'));
    } else {
      switch (tag) {
        ...
        case constants.TC_ENUM:
          ...
          break;
        default:
          this._reportError(errors.STREAM_INVALID_BLOCK_TYPE, '0x' + Buffer.from([tag]).toString('hex'));
          return null;
          break;
      }
    }
