...
class JObjStream {
  ...
  _readBlock(typeFilter) {
    let tag;
    this._reportPosition();
    if (!Array.isArray(typeFilter)) { throw (errors.FILTER_NOT_ARRAY); }
    tag = this._readTag();
    if (typeFilter.lastIndexOf(tag) === -1) {
      this._reportError(errors.STREAM_BAD_BLOCK_DATA, `0x${Buffer.from([type]).toString('hex')}`);
    } else {
      switch (tag) {
        ...
        case constants.TC_ENUM:
          ...
          break;
        default:
          this._reportError(errors.STREAM_INVALID_BLOCK_TYPE, `0x${Buffer.from([type]).toString('hex')}`);
          return null;
      }
    }
  }
