...
class JObjStream {
  ...
  _getObjDefinition(datatype) {
    ...
    switch (btype) {
      case constants.TC_STRING: {
        const innerCount = this._readCount();
        val.objType.type = 'string';
        val.objType.value = this._readName(innerCount);
  ...
  _readObjectData(objType) {
    ...
    const val = this._readBlock([constants.TC_OBJECT, constants.TC_NULL]);
  ...
  _readData(field) {
    switch (field.type) {
      ...
      case constants.DATATYPE_SHORT: {
        ...
        if (val & 0x8000) { return (0xFFFF0000 | (val & 0x0000FFFF)); }
        return val;
      }
  ...
  _readClassFlags() {
    ...
    if (flag & constants.SC_WRITE_METHOD) { flags.writeMethod = true; }
    if (flag & constants.SC_SERIALIZABLE) { flags.serializable = true; }
    if (flag & constants.SC_EXTERNALIZABLE) { flags.externalizable = true; }
    if (flag & constants.SC_BLOCK_DATA) { flags.blockData = true; }
    if (flag & constants.SC_ENUM) { flags.enum = true; }
    this._reportPosition();
    return flags;
  }
  ...
  _readClassDesc() {
    ...
    let obj = {};
    const count = this._readCount();
    classDef.name = this._readName(count);
    classDef.type = 'Class';
